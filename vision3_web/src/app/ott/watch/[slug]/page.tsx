// vision3_web/src/app/ott/watch/[slug]/page.tsx
...
export default async function WatchPage({ params, searchParams }: PageProps) {
  ...
  const hallHref = `/ott/${title.mode}/${title.genre}`;
  const originReturnTo = resolvedSearchParams.returnTo;
  const backHref = originReturnTo || hallHref;

  const currentWatchPath = `/ott/watch/${title.slug}?episode=${selectedEpisode.number}${
    originReturnTo ? `&returnTo=${encodeURIComponent(originReturnTo)}` : ""
  }`;

  const castPageHref = `/ott/watch/${title.slug}/cast?episode=${
    selectedEpisode.number
  }&returnTo=${encodeURIComponent(currentWatchPath)}`;

  const episodesPageHref = `/ott/watch/${title.slug}/episodes?episode=${
    selectedEpisode.number
  }&returnTo=${encodeURIComponent(currentWatchPath)}`;

  const writerPageHref = `/ott/watch/${title.slug}/writer?episode=${
    selectedEpisode.number
  }&returnTo=${encodeURIComponent(currentWatchPath)}`;
            ...
            {title.mode === "series" ? (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Link
                    href={`/ott/${title.mode}/${title.genre}`}
                    className="rounded-full border px-3 py-1 text-[11px] font-semibold transition"
                    style={hallButtonStyle}
                  >
                ...
                <div className="mt-5 grid gap-5 md:grid-cols-3">
                  {movieRecommendations.map((relatedTitle) => (
                    <Link
                      ...
                      href={`/ott/watch/${relatedTitle.slug}?returnTo=${encodeURIComponent(
                        currentWatchPath
                      )}`}
